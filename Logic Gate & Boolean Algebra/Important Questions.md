Ques 1. Design All Logic Gates using the Universal Logic Gate- (NAND and NOR Gates).

**Shortcut Table For Number Of Universal Gates Required**
*| Logic Gates | NAND | NOR  |
| NOT GATE    |  1   |  1   |
| AND GATE    |  2   |  3   |
| OR GATE     |  3   |  2   | 
| XOR GATE    |  4   |  5   |
| XNOR GATE   |  5   |  4   |* 

Ques 2. Inverter using XOR 
        Inverter using XNOR 

Ques 3. Design 3 Input XOR using 2 Input XNOR Gate.

Ques 4. Design 3 Input XNOR using 2 Input XNOR Gate.

Ques 5. 

Ques 3. 3 Input AND Gate using 2 Input AND Gate 

Ques 4. 3 Input XNOR Gate using 2 Input XNOR Gate. 

Ques 5. 3 Input Binary to Gray and Gray to Binary. 

Ques 6. 5-Bit Palindrome Checker Circuit 

*If output is 1 then it is Palindrome, If it is 0 then it is not Palindrome*
